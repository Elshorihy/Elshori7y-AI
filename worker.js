...
async function chat(request, env) {
  ...
  if (!apiKey) return json({ error: 'لم يتم إعداد GEMINI_API_KEY على Cloudflare.' }, 500);
  ...
  try {
    ...
    return json({ error: 'Gemini أرسل استجابة غير صالحة.' }, 502);
  }

  if (!upstream.ok) {
    return json({ error: data?.error?.message || 'حدث خطأ من Gemini.' }, upstream.status);
  }
